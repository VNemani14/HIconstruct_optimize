...
for bid=1:Nbearing % bearing number
    load("YLPaperFeaturesAcc_B_"+string(bid)+".mat");
...
for cv = 1:ncv
    ...
    %pearson corr
    [pr_corr_Pr_test(cv), pr_cutoff_vec(cv,:), pr_metric_vec(cv,:)]=get_probability(mypr_corrf(testb), [0.8, 1.0]);
    %spearman corr
    [sp_corr_Pr_test(cv), sp_cutoff_vec(cv,:), sp_metric_vec(cv,:)]=get_probability(mysp_corrf(testb), [0.8, 1.0]);
    %old_mon
    ...
    %robustness
    [rb_corr_Pr_test(cv), rb_cutoff_vec(cv,:), rb_metric_vec(cv,:)]=get_probability(myrbf(testb), [0.85, 1.0]);
