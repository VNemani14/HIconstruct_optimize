...
for mybid=1:15
    load("../01_Bearing_raw_data/bearing"+ num2str(mybid) +'.mat')
    ...
    for mytime=1:ntime
            ...
            FTh(mytime,1)=mean(vib);  % max vibration
            ...
            si=1;ei=floor(3200/df);
            RS(mytime,3) = get_RSFeature(ALLP(:,si:ei));
            si=floor(3200/df);ei=floor(6400/df);
            RS(mytime,4) = get_RSFeature(ALLP(:,si:ei));
            si=floor(6400/df);ei=floor(9600/df);
            RS(mytime,5) = get_RSFeature(ALLP(:,si:ei));
            si=floor(9600/df);ei=floor(12800/df);
            ...
            %time domain features
            FTv(mytime,1)=mean(vib);  % max vibration
    ...
    mybid
    Fnet = [RS, RSv];
    save("YLPaperFeaturesAcc_B_"+string(mybid)+".mat",'Fnet')
