...
parfor cv = 1:ncv
    nlhs = 1000; %number of LHS samples for optimization - this will be doubled in the code
    testb =  TESTB(cv,:);
    ...
    sigmaEOL=zeros(nfeatures,1); mon_violation_count=zeros(nfeatures,1); myfsnr=zeros(nfeatures,1);
    for nf = 1:nfeatures
        [sigmaEOL(nf), mon_violation_count(nf)]  = get_othermetrics(AllFeatures(nf,trainb), FPT(trainb), Fmax(nf), Fmin(nf));
        myfsnr(nf)  = get_snr(AllFeatures(nf, trainb), FPT(trainb));
    end

    mysnr_index = myfsnr > quantile(myfsnr, 0.4); % top snr based features
    mon_violation_count_index = mon_violation_count < quantile(mon_violation_count, 0.6); % top mon violations based features

    select_features = (mysnr_index & mon_violation_count_index);
    ...
    for mylhs = 1:nlhs
        ...
        %spearman corr
        [sp_corr_Pr(mylhs),~, ~]=get_probability(sp_corrf(mylhs, trainb), [0.8, 1.0]);
        %new monotonicity
        [new_mon_Pr(mylhs), ~, ~]=get_probability(new_monf(mylhs, trainb), [0.5, 1.0]);
        %SNR
        obj1(mylhs)  = get_snr(ALLHI(mylhs, trainb), FPT(trainb));

        [~, obj2(mylhs)]  = get_othermetrics(ALLHI(mylhs,trainb), FPT(trainb), 1, 0);
    end
    [max_val,lhs_max_ind] = max(obj1);  % Just single objective here. 
    w_best = Wts(lhs_max_ind,:);
%     HI_test = ALLHI(lhs_max_ind,testb);
%     save('HI_LIU2','HI_test');
    ...
    %pearson corr
    [pr_corr_Pr_test(cv), pr_cutoff_vec(cv,:), pr_metric_vec(cv,:)]=get_probability(pr_corrf(lhs_max_ind,testb), [0.8, 1.0]);
    %spearman corr
    [sp_corr_Pr_test(cv), sp_cutoff_vec(cv,:), sp_metric_vec(cv,:)]=get_probability(sp_corrf(lhs_max_ind,testb), [0.8, 1.0]);
    %old_mon
    [old_mon_Pr_test(cv), old_mon_cutoff_vec(cv,:), old_mon_metric_vec(cv,:)]=get_probability(old_monf(lhs_max_ind,testb), [0.5, 1.0]);
    %new_mon
    [new_mon_Pr_test(cv), new_mon_cutoff_vec(cv,:), new_mon_metric_vec(cv,:)]=get_probability(new_monf(lhs_max_ind,testb), [0.5, 1.0]);
    %robustness
    [rb_corr_Pr_test(cv), rb_cutoff_vec(cv,:), rb_metric_vec(cv,:)]=get_probability(rbf(lhs_max_ind,testb), [0.85, 1.0]);
...
save('optimization_LIU2_LHS.mat')
