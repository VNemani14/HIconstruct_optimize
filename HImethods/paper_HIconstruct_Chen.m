% Paper:
% Chen, Zhen, et al. "A Health Index Construction Framework for Prognostics
% Based on Feature Fusion and Constrained Optimization." IEEE Transactions
% on Instrumentation and Measurement 70 (2021): 1-15.
clear all
FPT=[79,55,60,106,26,456,50,316,32,123,2404,2450,343,1420,8]; %First Prediction Time
Nbearing = 15;
...
for cv = 1:ncv
    nlhs = 10; %number of LHS samples for optimization
    testb =  TESTB(cv,:);
    ...
    %pearson corr
    [pr_corr_Pr_test(cv), pr_cutoff_vec(cv,:), pr_metric_vec(cv,:)]=get_metaprobability(pr_corrf(testb), [0.8, 1.0]);
    %spearman corr
    [sp_corr_Pr_test(cv), sp_cutoff_vec(cv,:), sp_metric_vec(cv,:)]=get_metaprobability(sp_corrf(testb), [0.8, 1.0]);
    %old_mon
    ...
    %robustness
    [rb_corr_Pr_test(cv), rb_cutoff_vec(cv,:), rb_metric_vec(cv,:)]=get_metaprobability(robf(testb), [0.85, 1.0]);
